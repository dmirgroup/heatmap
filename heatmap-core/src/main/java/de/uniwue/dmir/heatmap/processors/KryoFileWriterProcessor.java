...
public class KryoFileWriterProcessor<TTile> 
extends AbstractFileWriterProcessor<TTile> {

	private Kryo kryo;
	
	public KryoFileWriterProcessor(
			String parentFolder,
			IFileStrategy fileStrategy) {
		super(parentFolder, fileStrategy, "bin", false);
		this.kryo = new Kryo();
	}
	...
	public void process(TTile tile, TileSize tileSize, TileCoordinates coordinates) {
		...
		try {
			...
			Output output = new Output(super.getOutputStream(coordinates));
			this.kryo.writeClassAndObject(output, tile);
			output.close();
	...
	implements IFileWriterProcessorFactory<TTile> {
		...
		public ITileProcessor<TTile> getInstance(String parentFolder) {
			return new KryoFileWriterProcessor<TTile>(
					parentFolder, 
					this.fileStrategy);
		}
